...
def operator_corchete(left, right, tabs, primerosProduccion):
    ...
    if isinstance(right, tuple):
        ...
        root = root + ['\t' * tabs + 'if self.currentToken in ["' + right.Valor + '"]:'] + ['\t' * tabs + '\tself.expect("' + right.Valor + '")']
...
def operator_llave(left, right, tabs, primerosProduccion):
    ...
    if isinstance(right, tuple):
        ...
        root = root + ['\t' * tabs + 'while self.currentToken in ["' + right.Valor + '"]:'] + ['\t' * tabs + '\tself.expect("' + right.Valor + '")']
    return (root, first), tabs
